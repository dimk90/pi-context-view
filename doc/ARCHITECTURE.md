...
- `/context config` is the explicit create-only action: it is skipped outside `ctx.mode === "tui"`, writes every default through one atomic `O_EXCL` create, and never overwrites or modifies an existing path;
